...
def save_assessment_church_target_bars(df, start_date, end_date, targets_df):
    ...
    # Safety Check: If no assessment data exists for the period
    if actual_data.empty:
        return "Regarding assessment income, no data was recorded for this period."
    ...
    # 5. Iterate through each church to calculate Target vs. Actual performance
    for church in [c for c in churches if c in income_targets['Entity'].values]:
        ...
        # Stewardship Signal Calculation
    ...
    # Fallback if no matching targets were found for the contributing churches
    if not performance_msgs:
...
def save_minister_stipend_bars(df, start_date, end_date, targets_df):
    ...
    # 3. Iterate through Ministers to calculate performance (Plotting removed)
    for minister in [m for m in ministers if m in stipend_targets['Entity'].values]:
        # Get target for this minister
        m_target = stipend_targets[(stipend_targets['Entity'] == minister)]['monthly_target'].values[0]
        
        # Calculate Average Performance for the message (Added division-by-zero safeguard)
        avg_paid = actuals[minister].mean()
        perf_pct = (avg_paid / m_target) * 100 if m_target > 0 else 0
    ...
    if performance_summaries:
        final_msg = "On average, the " + " and ".join(performance_summaries) + "."
    else:
        final_msg = "No ministerial stipend targets found for this period."
